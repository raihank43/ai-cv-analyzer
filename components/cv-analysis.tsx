...
export default function CVAnalysisDisplay({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'aspects' | 'keywords' | 'recommendation'>('overview');
  ...
  const aspectData = {
    ...
    professionalSummary: { 
      icon: FileText, 
      title: language === 'id' ? "Ringkasan Profesional" : "Professional Summary",
      description: language === 'id'
    ...
    educationCertification: { 
      icon: GraduationCap, 
      title: language === 'id' ? "Pendidikan & Sertifikasi" : "Education & Certification",
      description: language === 'id'
    ...
    organizationalActivity: { 
      icon: Users, 
      title: language === 'id' ? "Aktivitas Organisasi" : "Organizational Activity",
      description: language === 'id'
    ...
    keywords: { 
      icon: Hash, 
      title: language === 'id' ? "Optimasi Kata Kunci" : "Keywords Optimization",
      description: language === 'id'
  ...
  const analyzeCV = async () => {
    if (!cvText.trim()) {
      setError(language === 'id' ? "Tidak ada teks CV untuk dianalisis" : "No CV text available for analysis");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || (language === 'id' ? "Analisis gagal" : "Analysis failed"));
      }
      ...
      setError(error instanceof Error ? error.message : (language === 'id' ? "Gagal menganalisis CV" : "Failed to analyze CV"));
    } finally {
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              <Label htmlFor="target-role">{language === 'id' ? 'Posisi Target (Opsional)' : 'Target Role (Optional)'}</Label>
              <Input
                id="target-role"
                placeholder={language === 'id' ? 'Misal: Software Engineer, Marketing Manager' : 'e.g., Software Engineer, Marketing Manager'}
                value={targetRole}
            ...
            <div className="space-y-2">
              <Label htmlFor="industry">{language === 'id' ? 'Industri (Opsional)' : 'Industry (Optional)'}</Label>
              <Input
                id="industry"
                placeholder={language === 'id' ? 'Misal: Teknologi, Keuangan, Kesehatan' : 'e.g., Technology, Finance, Healthcare'}
                value={industry}
            ...
            <div className="space-y-2">
              <Label htmlFor="language">{language === 'id' ? 'Bahasa Analisis' : 'Analysis Language'}</Label>
              <select
                id="language"
                value={language}
          ...
          >
            {isAnalyzing ? (
              <>
                ...
                {language === 'id' ? 'Menganalisis CV dengan AI...' : 'Analyzing CV with AI...'}
              </>
            ) : (
              <>
                <TrendingUp className="w-4 h-4 mr-2" />
                {language === 'id' ? 'Analisis CV dengan AI' : 'Analyze CV with AI'}
              </>
            )}
          </Button>
          ...
          {error && (
            <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
              <div className="flex items-center gap-2 text-red-800">
                ...
                <span className="font-medium">{language === 'id' ? 'Kesalahan Analisis' : 'Analysis Error'}</span>
              </div>
      ...
      {isAnalyzing && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-lg">{language === 'id' ? 'Memuat...' : 'Loading...'}</CardTitle>
              </CardHeader>
      ...
      {analysisResult && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
              ...
              <CardContent>
                <div className="text-center">
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2 mt-3">
                    <div 
                      className={`h-2 rounded-full transition-all duration-500 ${getScoreColorBg(analysisResult.overallScore)}`}
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-lg">{language === 'id' ? 'Navigasi' : 'Navigation'}</CardTitle>
              </CardHeader>
          ...
          <div className="lg:col-span-3">
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    {language === 'id' ? 'Ringkasan Analisis Keseluruhan' : 'Overall Analysis Overview'}
                  </CardTitle>
                  <CardDescription>
                    {language === 'id' ? 'Ringkasan performa CV Anda di semua aspek' : 'Summary of your CV performance across all aspects'}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(analysisResult.aspects).map(([key, aspect]) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className={`px-2 py-1 rounded-full text-xs font-medium border ${getBadgeVariant(aspect.score)}`}>
                              {aspect.score}%
                            </div>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
                            <div 
                              className={`h-2 rounded-full transition-all duration-500 ${getScoreColorBg(aspect.score)}`}
                              style={{ width: `${aspect.score}%` }}
                            ></div>
                          </div>
                          <p className="text-xs text-gray-500">
                            {data?.description || 
                             (language === 'id' ? 'Aspek penting dalam evaluasi CV' : 'Important aspect in CV evaluation')}
                          </p>
                        </div>
                      );
                    })}
            ...
            {activeTab === 'aspects' && (
              <div className="space-y-6">
                ...
                <Card>
                  <CardHeader className="pb-3">
                    <CardTitle>{language === 'id' ? 'Pilih Aspek untuk Ditinjau' : 'Select Aspect to Review'}</CardTitle>
                  </CardHeader>
                  <CardContent>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="text-xs font-medium">
                              {data?.title.split(' ')[0] || aspect.name.split(' ')[0]}
                            </div>
                            <div className={`text-xs px-1 py-0.5 rounded font-medium ${getBadgeVariant(aspect.score)}`}>
                              {aspect.score}%
                            </div>
                          </button>
                        );
                ...
                {activeAspect && analysisResult.aspects[activeAspect as keyof typeof analysisResult.aspects] && (
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center justify-between">
                        <span className="flex items-center gap-2">
                          {(() => {
                            const data = aspectData[activeAspect as keyof typeof aspectData];
                            const IconComponent = data?.icon || FileText;
                            return <IconComponent className="w-6 h-6" />;
                          })()}
                          {aspectData[activeAspect as keyof typeof aspectData]?.title || 
                           analysisResult.aspects[activeAspect as keyof typeof analysisResult.aspects].name}
                        </span>
                        <div className={`text-lg px-3 py-1 rounded-full border font-medium ${getBadgeVariant(
                          analysisResult.aspects[activeAspect as keyof typeof analysisResult.aspects].score
                        )}`}>
                          {analysisResult.aspects[activeAspect as keyof typeof analysisResult.aspects].score}/100
                        </div>
                      </CardTitle>
                      <CardDescription>
                        {aspectData[activeAspect as keyof typeof aspectData]?.description ||
                         (language === 'id' ? 'Aspek penting dalam evaluasi CV' : 'Important aspect in CV evaluation')}
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                      <div className="w-full bg-gray-200 rounded-full h-3">
                        <div 
                          ...
                          style={{ width: `${analysisResult.aspects[activeAspect as keyof typeof analysisResult.aspects].score}%` }}
                        ></div>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-gray-700 leading-relaxed bg-gray-50 p-4 rounded-lg">
                          {analysisResult.aspects[activeAspect as keyof typeof analysisResult.aspects].analysis}
                        </p>
                      </div>
                      ...
                      <div>
                        <h4 className="font-medium text-gray-900 mb-3 flex items-center gap-2">
                          ...
                          {language === 'id' ? 'Poin Tindakan' : 'Action Points'}
                        </h4>
                        <ul className="space-y-3">
                          {analysisResult.aspects[activeAspect as keyof typeof analysisResult.aspects].actionPoints.map((point, index) => (
                            <li
                              key={index}
                      ...
                      <div className="pt-4 border-t bg-blue-50 p-4 rounded-lg">
                        <h4 className="font-medium text-blue-900 mb-2 flex items-center gap-2">
                          <Target className="w-4 h-4" />
                          {language === 'id' ? 'Mengapa Hal Ini Penting' : 'Why This Matters'}
                        </h4>
                        <p className="text-blue-800 text-sm leading-relaxed">
                          {analysisResult.aspects[activeAspect as keyof typeof analysisResult.aspects].whyImportant}
                        </p>
                      </div>
                    </CardContent>
                  </Card>
            ...
            {activeTab === 'keywords' && analysisResult.extractedKeywords && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Key terms and phrases identified in your CV for ATS optimization
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  ...
                  <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <h4 className="font-medium text-blue-900 mb-2">💡 {language === 'id' ? 'Tips Optimasi ATS' : 'ATS Optimization Tips'}</h4>
                    <ul className="text-blue-800 text-sm space-y-1">
                      {language === 'id' ? (
                        <>
                          <li>• Sertakan kata kunci industri secara alami dalam konten Anda</li>
                          <li>• Gunakan akronim dan bentuk lengkap (misalnya, "AI" dan "Kecerdasan Buatan")</li>
                          <li>• Cocokkan kata kunci dari deskripsi pekerjaan yang Anda lamar</li>
                          <li>• Hindari penumpukan kata kunci - fokus pada integrasi alami</li>
                        </>
                      ) : (
                        <>
                          <li>• Include industry-specific keywords naturally in your content</li>
                          ...
                          <li>• Avoid keyword stuffing - focus on natural integration</li>
                        </>
                      )}
                    </ul>
                  </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    ...
                    {language === 'id' ? 'Rekomendasi Karier' : 'Career Recommendation'}
                  </CardTitle>
                  <CardDescription>
                    {language === 'id'
                    ...
                    <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                      ...
                      <ul className="text-green-800 text-sm space-y-1">
                        {Object.entries(analysisResult.aspects)
                          ...
                          .map(([key, aspect]) => (
                            <li key={key}>• {aspectData[key as keyof typeof aspectData]?.title || aspect.name}</li>
                          ))}
                      </ul>
                    </div>
